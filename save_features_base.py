...
def save_feature(args):
    ...
    labels = labels[:flag]
    f = h5py.File('data/base_feat/img_train_feat_{}.h5'.format(args.id), 'w')
    ...
    features = features[:flag, :]
    labels = labels[:flag]
    f = h5py.File('data/base_feat/img_val_feat_{}.h5'.format(args.id), 'w')
    f.create_dataset('feature_map', data=features)
    f.create_dataset('labels', data=labels)
    f.close()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model', default="ckpt/model/net_epoch_11.pth",
                        help='model to load')
