...
def base_obj_list(args, base_set, base_list, img_path):
    ...
    for i in range(len(base_list)):
        ...
        if args.cap != 0:
            train_num = min(args.cap, train_num)
...
def novel_obj_list_before_feat(args, novel_set, novel_list, img_path):
    ...
    output_path = os.path.join(os.path.join(args.root_dataset, args.output),
                               'novel_obj_train_before_feat{}.odgt'.format(suffix))
    f = open(output_path, 'w')
    f.write(result_train)
    f.close()
    output_path = os.path.join(os.path.join(args.root_dataset, args.output),
                               'novel_obj_val_before_feat{}.odgt'.format(suffix))
    f = open(output_path, 'w')
    f.write(result_val)
    f.close()
